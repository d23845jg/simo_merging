...
from model_merging.tallmask_utils import (
...
>>>>>>> Stashed changes
    ...
    if merge_config["name"] == "ties":
        ...
        merged_tv = ties_merging(
            flat_task_vectors, reset_thresh=merge_config["k"], merge_func=merge_func
        )
...
=======
        ...
        if merge_config["use_ties"]:
            print(f"Using TIES for constructing multi-task vector")
            merged_tv = ties_merging(
                flat_task_vectors, reset_thresh=20, merge_func=f"dis-sum"
            )
        else:
            print(f"Using Task Arithmetic for constructing multi-task vector")
            flat_task_vectors, _ = topk_values_mask(
        ...
        if merge_config["load_masks"]:
            # load tall masks directly from storage
            eval_masks = load_tall_mask(remove_keys, mm_config)
        else:
            print(f"=== Constructing TALL Mask ===")
            # construct tall masks
            eval_masks = construct_tall_mask(
                task_vectors, flat_task_vectors, merged_tv, remove_keys
            )
    elif merge_config["name"] == "consensus":  # consensus merging
        # construct consensus mask (assuming the TALL masks have already been constructed)
        consensus_mask = construct_consensus_mask(
            merge_config["prun_thre_k"], mm_config, remove_keys
        )
        # construct multi-task vector
        if merge_config["use_ties"]:
            merged_tv = ties_merging(
                flat_task_vectors, reset_thresh=20, merge_func="dis-sum"
            )
        else:
            flat_task_vectors, _ = topk_values_mask(
                flat_task_vectors, K=merge_config["k"], return_mask=False
...
=======
    ...
    task_specific_tv_state_dict = dict(
        chain(*(task_vectors[task].tau.items() for task in task_vectors))
    )
    mtl_task_vector = MTLTaskVector(
        theta=shared_tv_state_dict, tau=task_specific_tv_state_dict
    )
