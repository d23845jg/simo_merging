...
<<<<<<< Updated upstream
    ...
    def forward(self, prediction, target, ignore_index=0, **kwargs):
        mask = (torch.sum(target, dim=1, keepdim=True) != ignore_index).to(prediction.device)
        loss = torch.abs(prediction - target).masked_select(mask).sum() / (mask.sum() + self.eps)
